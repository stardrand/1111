...
<form class="form-horizontal" role="form" action="{{url('goods/update/'.$ret->g_id)}}" method="post" enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        <div class="col-sm-2">
        <select name="bid"  class="form-control" id="firstname">
            <option value="">--请选择--</option>
            @foreach($res as $k=>$v)
            <option value="{{$v->bid}}" @if($ret->bid==$v->bid)selected @endif>{{$v->bname}}</option>
            @endforeach
        </select>
        </div>
    </div>
    ...
    <div class="form-group">
        ...
        <div class="col-sm-2">
        <select name="cate_id" id="" class="form-control" id="firstname">
            <option value="">--请选择--</option>
            @foreach($date as $k=>$v)
            <option value="{{$v->cate_id}}" @if($ret->cate_id==$v->cate_id)selected @endif>{{str_repeat("——",$v->level)}}{{$v->cate_name}}</option>
            @endforeach
        </select>
        </div>
    </div>
    ...
    <div class="form-group">
        ...
        <div class="col-sm-2">
        <img src="http://1908upload.com/{{$ret->g_img}}" width="50">
    ...
    <div class="form-group">
        ...
        @foreach($ret->g_imgs as $k=>$v)
            <img src="http://1908upload.com/{{$v}}" width="50">
            @endforeach
